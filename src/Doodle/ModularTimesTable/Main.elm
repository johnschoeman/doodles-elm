    ...
    case msg of
        UpdateModulus modulus ->
            ...
            case maybeNextModulus of
                Just nextModulus ->
                    if nextModulus < maxModulus then
                        ( { model | modulus = nextModulus }, Cmd.none )
        ...
        UpdateMultiplier multiplier ->
            ...
            case maybeNextMultiplier of
                Just nextMultiplier ->
                    if nextMultiplier <= model.modulus then
                        ( { model | multiplier = nextMultiplier }, Cmd.none )
...
inputs { modulus, multiplier } =
    ...
    div [ class "flex space-x-2 justify-start lg:flex-col lg:space-x-0 lg:space-y-2" ]
        [ InputHelpers.numberInput "Modulus" "Max: 5000" "-" modulusText UpdateModulus
        , InputHelpers.numberInput "Multiplier" "Max: (Modulus - 1)" "-" multiplierText UpdateMultiplier
        ]
